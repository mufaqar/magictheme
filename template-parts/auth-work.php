...
// GET VENDOR ID
// --------------------
$profile_vendor_id = $args['vendor_id'] ?? 0;

if ( ! $profile_vendor_id ) {
    return;
}
?>
    ...
    <div class="container">
        <div class="row product_row">
            ...
            <aside class="col-lg-3 filter-sidebar">
                ...
                <!-- ARTIST (LOCKED TO PROFILE) -->
                <div class="filter-box active">
                    <h5 class="filter-title">Artist <span class="accordion-icon"></span></h5>
                    <div class="filter-content">
                        <?php
                        $vendor = get_user_by('id', $profile_vendor_id);

                        if ( $vendor ) :
                            $checked = (
                                empty($_GET['vendor']) ||
                                $_GET['vendor'] == $profile_vendor_id
                            ) ? 'checked' : '';
                        ?>
                            <label>
                                <input type="checkbox"
                                       class="filter-vendor"
                                       value="<?php echo esc_attr($profile_vendor_id); ?>"
                                       <?php echo $checked; ?>>
                ...
                <div class="side_cta" style="background: #17A2B814;">
                    ...
                    <a href="<?php echo esc_url( home_url('/sign-up') ); ?>" class="sign_btn">
                        <span>Sign up</span>
                    </a>
                </div>
            </aside>
            ...
            <div class="col-lg-9">
                ...
                $query_args = [
                    ...
                    'posts_per_page' => -1,
                    'post_status'    => 'publish',
                    'author'         => $profile_vendor_id, // 🔒 LOCK TO PROFILE VENDOR
                ];

                // Override vendor if filter explicitly used
                if ( ! empty($_GET['vendor']) ) {
                    $query_args['author'] = intval($_GET['vendor']);
                }
                ...
                <div class="sorting-bar align-items-center">
                    ...
                    <span>
                        ...
                        <select id="sort" class="orderby">
                            ...
                            <option value="date" <?php selected($_GET['orderby'] ?? '', 'date'); ?>>Latest</option>
